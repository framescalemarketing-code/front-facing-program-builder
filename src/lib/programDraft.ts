...
export type DepartmentAllowanceRow = {
  id: string;
  name: string;
...
export type CalculatorState = {
  ...
  addOns: AddOns;
  departmentAllowances?: DepartmentAllowanceRow[];
...
export type CalculatorStatePatch = Partial<CalculatorState> & {
  addOns?: AddOnsPatch;
...
export type DraftPatch = {
  ...
  calculator?: CalculatorStatePatch;
};
...
function normalizeDepartmentAllowances(
  rows: DepartmentAllowanceRow[] | undefined
): DepartmentAllowanceRow[] {
...
function mergeCalculator(current: CalculatorState, patch: CalculatorStatePatch | undefined): CalculatorState {
  if (!patch) return current;
...
export function mergeDraft(current: Draft, patch: DraftPatch): Draft {
  ...
  const calculatorPatch = isObject(patch.calculator)
    ? (patch.calculator as DraftPatch["calculator"])
    : undefined;
