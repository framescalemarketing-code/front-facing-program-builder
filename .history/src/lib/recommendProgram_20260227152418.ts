...
const ALL_COATINGS: Omit<CoatingRecommendation, "reason">[] = [
  ...
  },
  {
    ...
    description:
      "Includes both anti-reflective and anti-fog treatments.",
  },
  {
...
export function recommendProgram(
  ...
  if (isMultiLocation) {
    ...
    if (locationModel === "multi_across_regions") {
      if (coverageSizeBand === "1_30") {
        serviceTier = escalateTier(serviceTier, "Premier");
        rationale.push(
          "Location override: Cross-region small team → Premier.",
        );
      } else {
        serviceTier = "Enterprise";
        rationale.push(
    ...
    if (inGroupB && locationModel === "multi_across_regions") {
      ...
      rationale.push(
        "Location EU: Group B cross-region → Covered.",
      );
    }
  }
  ...
  if (usesOnsiteOrHybrid) {
    serviceTier = escalateTier(serviceTier, "Premier");
    rationale.push(
      "Service model: Onsite/Hybrid delivery → at least Premier.",
    );
  }
  ...
  if (budgetPreference) {
    ...
    if (beforeEu !== euPackage) {
      rationale.push(
        `Budget clamp (${constraints.label}) → EU ${euPackage}.`,
      );
    }
  }
  ...
  if (isMultiLocation && (serviceTier === "Essential" || serviceTier === "Access")) {
    serviceTier = "Premier";
    rationale.push(
      "Safety net: Multi-site must be Premier or higher.",
    );
  }
