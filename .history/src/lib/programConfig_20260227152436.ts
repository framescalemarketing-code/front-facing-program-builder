...
export type ProgramLocationModel = "single" | "multi_same_region" | "multi_across_regions";
...
export type ProgramConfig = {
  ...
  programProfile: {
    ...
    coverageSizeBand?: "1_30" | "31_60" | "61_100" | "101_250" | "251_500" | "500_plus";
...
export function deriveProgramConfigFromDraft(draft: Draft): ProgramConfig {
  ...
  const locationsCount = Array.isArray(draft.program.locations) ? draft.program.locations.length : 0;

  const derivedTier = deriveComplexityTier({
    employees: typeof draft.program.eligibleEmployees === "number" ? draft.program.eligibleEmployees : 0,
    locations: locationsCount,
    exposureCount: 0,
    deliverySignals: [],
    approvalSignals: [draft.builder.guidelines.approvalWorkflowEnabled ? "manager" : "none"],
  });
...
export function buildDraftPatchFromProgramConfig(config: ProgramConfig): DraftPatch {
  const allowanceScope = config.recommendedStructure.allowanceScope ?? "companywide";
  const selectedTier = (config.recommendedStructure.serviceTier ?? "") as ServiceTier | "";
  return {
    builder: {
      guidelines: {
        ...
        allowanceScope,
        approvalWorkflowEnabled: config.approvalModel.model === "manager" || config.approvalModel.model === "centralized_safety",
        restrictions: {
...
export function deriveComplexityTier(args: {
  ...
  const employees = Math.max(0, Number.isFinite(args.employees) ? args.employees : 0);
  const locations = Math.max(0, Number.isFinite(args.locations) ? args.locations : 0);
  const exposureCount = Math.max(0, Number.isFinite(args.exposureCount) ? args.exposureCount : 0);
  ...
  else if (args.deliverySignals.includes("onsite") && args.deliverySignals.includes("mail")) score += 2;
  ...
  if (score >= 8) return "enterprise_scale";
  if (score >= 5) return "multi_site_controlled";
