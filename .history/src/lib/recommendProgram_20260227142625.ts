...
function budgetConstraint(
  ...
  if (budgetPreference === "super_strict") {
    // Strictest budget: cap at Access tier for all sizes, allow Comfort EU
    return {
...
export function recommendProgram(
  ...
  if (isMultiLocation) {
    ...
    // Group B industries (healthcare, lab, utilities, public_sector)
    // need stronger EU packages for multi-site operations
    if (inGroupB) {
      euPackage = escalateEu(euPackage, "Covered");
      rationale.push("Escalation: Group B multi-site → EU at least Covered.");

      if (locationModel === "multi_across_regions") {
        serviceTier = escalateTier(serviceTier, "Enterprise");
        rationale.push("Escalation: Group B cross-region → Enterprise tier.");
      }
    }
