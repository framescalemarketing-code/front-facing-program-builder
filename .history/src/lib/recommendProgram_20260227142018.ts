...
export function recommendProgram(
  ...
  if (inGroupA && onlyGroupABaseExposures) {
    ...
    rationale.push(
      "Base: Group A with allowed exposures -> Compliance + size tier.",
    );
  } else if (inGroupB) {
    euPackage = "Comfort";
    serviceTier = "Access";
  ...
  if (isNoFormalProgram && coverageSizeBand === "1_30") {
    ...
    tierFromExplicitRule = true;
    rationale.push(
      "Special case: No formal program + 1 to 30 -> Compliance + Essential.",
    );
  }
  ...
  if (inGroupB && selectedAddOns.length >= 1) {
    ...
    tierFromExplicitRule = true;
    rationale.push(
      "Escalation: Group B with selected add-on(s) -> Tier Premier.",
    );
  }
