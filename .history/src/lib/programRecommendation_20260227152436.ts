...
> & {
  ...
  coverageSizeBand: NonNullable<ProgramConfig["programProfile"]["coverageSizeBand"]>;
...
function deriveDeliveryModel(
  ...
  if (setup.includes("mail_fulfillment")) {
    return {
      primary: "mail",
      notes: "Online ordering supports distributed teams without onsite visits.",
    };
  }
...
function deriveCoverageTypeFromSetup(setup: CurrentSafetySetup[]) {
  if (setup.includes("hybrid_eyewear")) return "prescription_and_plano" as const;
  if (
