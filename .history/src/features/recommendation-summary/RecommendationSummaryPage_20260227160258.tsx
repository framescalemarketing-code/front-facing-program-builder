                  ...
                  {activeCardSection === "snapshot" && (
                    <div className="p-6 space-y-6">
                      ...
                      <div className="flex flex-wrap gap-x-6 gap-y-3">
                        <div>
                          <span className="text-[10px] font-semibold uppercase tracking-widest text-slate-400 block">EU Package</span>
                          <span className="text-lg font-bold text-[#244093]">{selectedPackage ?? "—"}</span>
                        </div>
                        <div>
                          <span className="text-[10px] font-semibold uppercase tracking-widest text-slate-400 block">Service Tier</span>
                          <span className="text-lg font-bold text-[#244093]">{serviceTier ?? "—"}</span>
                        </div>
                        <div>
                          <span className="text-[10px] font-semibold uppercase tracking-widest text-slate-400 block">Coverage</span>
                          <span className="text-sm font-bold text-foreground">{coverageBand ? `${coverageBand} employees` : "—"}</span>
                        </div>
                        <div>
                          <span className="text-[10px] font-semibold uppercase tracking-widest text-slate-400 block">Posture</span>
                          <span className="text-sm font-bold" style={{ color: posture.accentColor }}>{posture.icon} {posture.label}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-x-8 gap-y-1 text-sm">
                        {workType && (
                          <span className="text-slate-500"><span className="font-medium text-slate-700">Industry:</span> {workType}</span>
                        )}
                        {locationModel && (
                          <span className="text-slate-500"><span className="font-medium text-slate-700">Locations:</span> {locationModel}{locations.length > 0 ? ` (${locations.length})` : ""}</span>
                        )}
                        {deliveryModel && (
                          <span className="text-slate-500"><span className="font-medium text-slate-700">Delivery:</span> {deliveryModel}</span>
                        )}
                        {approvalModel && (
                          <span className="text-slate-500"><span className="font-medium text-slate-700">Approval:</span> {approvalModel}</span>
                        )}
                      </div>
                      ...
                      {(programConfig.coatingRecommendations ?? []).length > 0 && (
                        <div>
                          <span className="text-[10px] font-semibold uppercase tracking-widest text-slate-400 mr-2">Recommended Coatings</span>
                      ...
                      {/* What happens next */}
                      <div className="rounded-lg bg-emerald-50/60 px-5 py-4 -mx-1">
