...
function postureCard(tier: ProgramComplexityTier) {
  ...
  > = {
    foundational: {
      ...
      explanation:
        "Your organization is at the stage where establishing a clear, enforceable safety eyewear standard is the priority. Many teams your size operate without formal structure — this recommendation builds that foundation so eligibility, ordering, and compliance are handled consistently from the start. It's designed to give you a solid starting point that grows with you.",
      icon: "🏗️",
      accentColor: "#3b82f6",
    },
    structured: {
      ...
      explanation:
        "Your program has moved past basic compliance — the challenge now is keeping day-to-day execution consistent as your team grows. At this stage, ordering bottlenecks, missed reorders, and unclear eligibility become the biggest friction points. This recommendation is tuned for operational momentum: faster workflows, reliable employee access, and the controls to keep things running smoothly.",
      icon: "⚙️",
      accentColor: "#059669",
    },
    multi_site_controlled: {
      ...
      explanation:
        "With multiple locations and a growing workforce, your organization is at the point where program consistency can't rely on manual coordination alone. Standards start to drift between sites, exceptions pile up, and execution becomes unpredictable without the right infrastructure. This recommendation keeps your program aligned across locations and limits exception drift — so growth doesn't create gaps.",
      icon: "🗺️",
      accentColor: "#d97706",
    },
    enterprise_scale: {
      ...
      explanation:
        "Your organization operates at a scale where governance, dedicated support, and cross-site consistency are foundational requirements — not nice-to-haves. Programs at this level need specialist partnership, leadership-level visibility, and resilient processes that perform under the complexity of multiple regions, roles, and compliance environments. This recommendation is built for that reality.",
      icon: "🏢",
      accentColor: "#7c3aed",
    },
  };
...
function ProgramSummaryCard(props: {
  ...
  const hasCoatings = props.coatingRecommendations.length > 0;
  ...
  return (
    <div className="rounded-xl border border-border bg-white shadow-sm overflow-hidden">
      {/* Card Header - Visual Program Identity — brand navy with subtle gradient */}
      <div
        className="relative p-6 pb-5 overflow-hidden"
        style={{
          background:
            "linear-gradient(135deg, #244093 0%, #2971b5 60%, #5e97dd 100%)",
        }}
      >
      ...
      <div className="p-5">
        ...
        {activeSection === "coatings" && hasCoatings && (
          <div className="space-y-4">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-base" aria-hidden="true">💡</span>
              <div>
                <h4 className="text-sm font-bold text-foreground">Did You Consider?</h4>
                <p className="text-xs text-muted-foreground">Based on your industry and exposure profile, these coatings could improve your program.</p>
              </div>
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              {props.coatingRecommendations.map((coating) => (
                <div
                  key={coating.id}
                  className="rounded-lg border border-border bg-gradient-to-br from-white to-slate-50/80 p-4 transition hover:border-primary/30 hover:shadow-sm"
                >
                  <h5 className="text-sm font-semibold text-foreground mb-1">{coating.label}</h5>
                  <p className="text-xs text-muted-foreground leading-relaxed mb-2">{coating.description}</p>
                  <p className="text-xs text-primary/80 font-medium leading-relaxed italic">{coating.reason}</p>
                </div>
              ))}
            </div>
          </div>
