    ...
    <section aria-labelledby="recommendation-congratulations-title">
      <header className="relative overflow-hidden bg-gradient-to-br from-[#2971b5] via-[#4a8fd4] to-[#6baee8] text-white">
        <svg className="pointer-events-none absolute inset-0 h-full w-full opacity-10" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
          <defs>
            <pattern id="confetti-grid" width="40" height="40" patternUnits="userSpaceOnUse">
        ...
        <div className="relative mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
          <div className="grid gap-8 lg:grid-cols-2 lg:items-center">
            <div>
              ...
              <h1 id="recommendation-congratulations-title" className="text-3xl font-bold tracking-tight text-white sm:text-4xl">
              ...
              <p className="mt-3 max-w-md text-base leading-relaxed text-white/90">
                An OSSO program specialist will review your profile and reach out to confirm next steps. Nothing moves until you review it together.
              </p>
            </div>

            <div className="hidden overflow-hidden rounded-2xl lg:block" aria-hidden="true">
              <img
            ...
            <div className="space-y-5 lg:col-span-2">
              <article className="overflow-hidden rounded-xl border border-border bg-card">
                <div className="border-b border-border bg-secondary/30 px-5 py-4">
                  ...
                  <p className="mt-0.5 text-xs text-muted-foreground">Three steps, no surprises</p>
                </div>
            ...
            <div className="space-y-5">
              {(companyName || contactEmail) && (
                <article className="rounded-xl border border-border bg-card p-5">
                  <h3 className="mb-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Submitted for</h3>
                  {companyName && <p className="text-base font-bold text-foreground">{companyName}</p>}
                  {contactEmail && <p className="mt-0.5 text-sm text-muted-foreground">{contactEmail}</p>}
                </article>
              )}
              ...
              <article className="rounded-xl border border-border bg-secondary/20 p-5">
                <div className="flex items-start gap-3">
                  <div className="mt-0.5">
                    <svg viewBox="0 0 20 20" className="h-5 w-5 text-foreground" aria-hidden="true">
                      <path
                        fill="currentColor"
                  ...
                  <div>
                    <p className="mb-1 text-xs font-semibold text-foreground">Your data is safe</p>
                    <p className="text-xs leading-relaxed text-muted-foreground">
                      Your recommendation is only shared with your assigned OSSO specialist. Nothing is sold, shared with third parties, or used outside your program setup.
                    </p>
                  </div>
                </div>
              </article>
