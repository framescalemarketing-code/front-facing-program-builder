...
const STEPS: WizardStep[] = [
  {
    ...
    heroSubtitle:
      "Your contact becomes the anchor point for everything that follows",
  ...
  },
  {
    ...
    heroSubtitle:
      "Accuracy here is what separates a tailored program from a generic one",
  ...
  },
  {
    ...
    heroSubtitle:
      "We build around how your program works today, not an assumed starting point",
    progressLabel: "Setup",
  },
  {
    id: "budget",
    ...
    heroSubtitle:
      "The most useful recommendation starts with where you actually are",
...
const WORK_TYPE_OPTIONS: Array<{
...
}> = [
  {
    ...
    helper:
      "Shift floors and production lines where durable daily wear and fast remake access keep operations moving — and programs current.",
  },
  {
    value: "construction",
    label: "Construction & Field Work",
    helper:
      "Active sites with impact hazards, debris exposure, and changing light — programs need to flex around crews without adding coordination overhead.",
  },
  {
    value: "utilities",
    label: "Utilities & Field Services",
    helper:
      "Regional field teams where variable conditions, territory routing, and compliance documentation all have to work at the same time.",
  },
  {
    value: "warehouse",
    label: "Warehouse & Distribution",
    helper:
      "High-throughput distribution environments where dependable daily wear and simple reorder access prevent compliance gaps without adding admin overhead.",
  },
  {
    value: "healthcare",
    label: "Healthcare & Clinical",
    helper:
      "Clinical environments where optical clarity, cleaning-cycle durability, and role-based splash protection determine whether protection actually gets used.",
  },
  {
    value: "public_sector",
    label: "Public Sector & Municipal",
    helper:
      "Multi-department environments where procurement standards, role eligibility, and documentation requirements need to be enforced consistently across functions.",
  },
  {
    value: "laboratory",
    label: "Laboratory & Research",
    helper:
      "Controlled research and lab settings where splash protection, anti-fog reliability, and protocol-aligned options aren't preferences — they're required by the work.",
  },
  {
    value: "other",
    label: "Specialized / Mixed Environment",
    helper:
      "Operations with exposure profiles or policy needs that don't fit standard categories — custom structures and edge cases built in from the start.",
  },
];
...
const COVERAGE_BANDS: Array<{
...
}> = [
  {
    ...
    label: "1 to 30",
    helper:
      "Small team with direct coordination — one owner, simple eligibility, and a structure tight enough to run without a playbook.",
  },
  {
    value: "31_60",
    label: "31 to 60",
    helper:
      "Growing volume where ad hoc coordination starts to break down. Scheduling rhythm and a repeatable intake process start paying off here.",
  },
  {
    value: "61_100",
    label: "61 to 100",
    helper:
      "Multiple supervisors feeding requests into the same program — missed handoffs become the main delay source at this size.",
  },
  {
    value: "101_250",
    label: "101 to 250",
    helper:
      "Programs running across departments or buildings where approvals stall without clear routing and informal rules stop working.",
  },
  {
    value: "251_500",
    label: "251 to 500",
    helper:
      "Distributed workforce where onboarding, reorders, and exceptions are running simultaneously — coordination is continuous work at this scale.",
  },
  {
    value: "500_plus",
    label: "500+",
    helper:
      "Enterprise operations where scheduling and fulfillment run as parallel workflows across sites, shifts, and role families — governance and visibility are required infrastructure, not upgrades.",
  },
];
...
const LOCATION_MODELS: Array<{
...
}> = [
  {
    ...
    helper:
      "One site, one owner, one execution path — the cleanest structure to build and prove before expanding.",
  },
  {
    ...
    helper:
      "Multiple sites close enough to share service windows and playbooks — coordination stays manageable when sites move together.",
  },
  {
    ...
    helper:
      "Sites spread across regions that need strong governance to stay consistent — where the program can't rely on informal coordination anymore.",
  },
  {
    id: "multi_complex",
    value: "multi_across_regions",
    ...
    helper:
      "Complex, international, or mixed-footprint operations — centralized policy with local execution, built to stay consistent regardless of geography.",
  },
];
...
const EXPOSURE_OPTIONS: Array<{
...
}> = [
  {
    ...
    label: "High Impact",
    helper:
      "Machine-adjacent and tool-heavy work where impact-rated frames and secure retention aren't optional — they're the standard.",
  },
  {
    value: "dust_debris",
    label: "Dust or Debris",
    helper:
      "Grinding, sanding, and cutting environments where airborne particles degrade lenses fast and interrupt visibility during the shift.",
  },
  {
    value: "chemical_splash",
    label: "Chemical Splash",
    helper:
      "Chemical handling, mixing, or lab work where fluid contact risk requires splash-oriented designs, not standard safety frames.",
  },
  {
    value: "outdoor_glare",
    label: "Outdoor Glare",
    helper:
      "Sustained outdoor exposure where glare isn't a nuisance — it reduces hazard awareness and drives employees to remove eyewear.",
  },
  {
    value: "fog_humidity",
    label: "Fog or Humidity",
    helper:
      "Cold storage, humid processing, or hot environments where fogging interrupts task flow and makes employees remove their eyewear mid-shift.",
  },
  {
    value: "indoor_outdoor_shift",
    label: "Indoor and Outdoor Shift Changes",
    helper:
      "Roles that move between facility and yard, dock and floor, or office and field — where slow light adaptation creates real safety lag.",
  },
  {
    value: "screen_intensive",
    label: "Screen Intensive Tasks",
    helper:
      "Extended screen work where visual fatigue builds across the shift and reduces comfort, focus, and sustained eyewear use.",
  },
  {
    value: "temperature_extremes",
    label: "Temperature Extremes",
    helper:
      "Foundry floors, cold storage, outdoor Texas summers — temperature swings that stress lens performance and make employees want to ditch their eyewear.",
  },
];
...
const CURRENT_SETUP_SECTIONS: Array<{
...
}> = [
  {
    ...
    helper:
      "How is safety eyewear currently funded and authorized for your team?",
    options: [
      {
        value: "no_formal_program",
        label: "No Formal Program",
        helper:
          "Purchases happen as needed, there's no standard, and the safety manager ends up fielding questions individually every time.",
      },
      {
        value: "voucher",
        label: "Voucher / Reimbursement",
        helper:
          "Employees follow a defined voucher or reimbursement process — there's a policy, but execution depends on employees following it correctly.",
      },
      {
        value: "vendor_optometry_partnership",
        label: "Vendor / Optometry Partnership",
        helper:
          "Safety eyewear runs through an existing vision benefit or optometry partner — coverage exists but may not be optimized for safety compliance.",
      },
    ],
  },
  {
    ...
    options: [
      {
        ...
        helper:
          "Orders wait for a manager or safety reviewer before they move — adds control, but adds queue time.",
      },
    ],
  },
  {
    ...
    options: [
      {
        ...
        helper:
          "Employees order directly through approved channels — fastest access, but requires strong guardrails to stay within policy.",
      },
      {
        value: "onsite_events",
        label: "Onsite Events",
        helper:
          "Scheduled onsite visits where employees are fitted and ordered in a controlled setting — higher accuracy, better adoption.",
      },
      {
        value: "mail_fulfillment",
        label: "Online Ordering",
        helper:
          "Employees order online and receive directly — removes site scheduling, but requires clear instructions and strong support for remakes.",
      },
      {
        value: "hybrid_delivery",
        label: "Hybrid",
        helper:
          "Onsite events for primary or high-priority groups, online for others — more to coordinate, but matches delivery to how the workforce actually operates.",
      },
    ],
  },
  {
    ...
    options: [
      {
        ...
        helper:
          "Workers who need vision correction get custom safety eyewear — the highest-adoption path because the glasses actually work for them.",
      },
      {
        value: "otg_non_prescription_eyewear",
        label: "Bulk Over the Glasses (OTG)",
        helper:
          "OTG safety frames issued over personal prescription glasses — fast to deploy, but comfort and compliance drop fast if fit isn't right.",
      },
      {
        value: "non_prescription_safety_eyewear",
        label: "Non Prescription Safety Eyewear",
        helper:
          "Standard safety frames for workers who don't need vision correction — simple to manage, cost-effective, and easy to reorder.",
      },
      {
        value: "hybrid_eyewear",
        label: "Hybrid Model",
        helper:
          "Different pathways by role — prescription for some, non-prescription or OTG for others. More accurate coverage, more to administer.",
      },
    ],
  },
];
...
export function RecommendationIntakePage({
  ...
  function goToStep(nextIndex: number) {
    setError("");
    if (nextIndex > 0) setFramingDismissed(true);
        ...
        <SectionWrap>
                ...
                <div className="rounded-xl overflow-hidden border border-border bg-card">
                  ...
                  <div className="p-5">
                    <p className="text-base font-bold text-foreground">
                      Build your program recommendation in about five minutes.
                    </p>
                    <p className="mt-2 text-sm text-muted-foreground">
                      Answer seven questions about your workforce, your
                      exposures, and how your program runs today — we'll build a
                      recommendation and connect you with an OSSO program
                      specialist to review it together.
                    </p>
                    <p className="mt-3 inline-flex items-center gap-1.5 rounded-full border border-primary/20 bg-primary/5 px-3 py-1 text-xs font-medium text-primary">
                      ...
                      Nothing is finalized here. This is a starting point, not a
                      contract.
                    </p>
                  </div>
                </div>
          ...
          {draft.programConfig?.active ? (
            <div className="mt-8 rounded-md border border-border bg-card p-4 text-xs text-muted-foreground">
              A recommended configuration is stored for this session. You can
...
function buildGuidance(args: {
  ...
  if (stepId === "budget") {
    ...
    return {
      ...
      sections: guidanceSections(
        ...
        {
          title: "How this shapes the recommendation",
  ...
  return {
    ...
    sections: guidanceSections(
      {
        title: "How this shapes the operating structure",
        body: copy.structure,
      },
      {
