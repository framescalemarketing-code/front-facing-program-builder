...
test(
  "1) Manufacturing 1 to 30 with high impact and dust, no program, no service model, no budget",
  () => {
...
test(
  "2) Manufacturing 61 to 100 with high impact and dust -> Compliance and Access",
  () => {
    const result = recommendProgram(
      makeInputs({
        workType: "manufacturing",
...
test(
  "3) Manufacturing 251 to 500 with high impact and dust -> Compliance and Premier",
  () => {
    const result = recommendProgram(
      makeInputs({
        workType: "manufacturing",
...
test(
  "4) Manufacturing with fog and screen intensive hazards infers Anti fog and Blue light",
  () => {
    const result = recommendProgram(
      makeInputs({
        workType: "manufacturing",
...
test(
  "10) Lean Essentials clamps down scenario that would otherwise be Covered and Enterprise",
  () => {
    const result = recommendProgram(
      makeInputs({
        workType: "healthcare",
...
test("11) Performance First posture always resolves to Covered and Enterprise", () => {
  const result = recommendProgram(
    makeInputs({
      workType: "manufacturing",
      coverageSizeBand: "1_30",
