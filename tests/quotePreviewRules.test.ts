...
} from "../src/lib/quotePreviewRules.js";
...
test("resolves combined department EU print mode when allowance and add-ons are open", () => {
  const state = resolveDepartmentBreakdownPrintState({
    print: true,
    canShowDepartmentAllowanceBreakdown: true,
    canShowDepartmentAddOnsBreakdown: true,
    canShowDepartmentServiceBreakdown: true,
    showDepartmentAllowanceBreakdown: true,
    showDepartmentAddOnsBreakdown: true,
    showDepartmentServiceBreakdown: true,
  });

  assert.equal(state.showPrintDepartmentEuBreakdown, true);
  assert.equal(state.showPrintDepartmentAllowanceBreakdown, false);
  assert.equal(state.showPrintDepartmentAddOnsBreakdown, false);
  assert.equal(state.showPrintDepartmentServiceBreakdown, true);
  assert.equal(state.showPrintAddOnsServiceAlignmentMode, false);
});

test("resolves add-ons and service alignment print mode when allowance is closed", () => {
  const state = resolveDepartmentBreakdownPrintState({
    print: true,
    canShowDepartmentAllowanceBreakdown: true,
    canShowDepartmentAddOnsBreakdown: true,
    canShowDepartmentServiceBreakdown: true,
    showDepartmentAllowanceBreakdown: false,
    showDepartmentAddOnsBreakdown: true,
    showDepartmentServiceBreakdown: true,
  });

  assert.equal(state.showPrintDepartmentEuBreakdown, false);
  assert.equal(state.showPrintDepartmentAllowanceBreakdown, false);
  assert.equal(state.showPrintDepartmentAddOnsBreakdown, true);
  assert.equal(state.showPrintDepartmentServiceBreakdown, true);
  assert.equal(state.showPrintAddOnsServiceAlignmentMode, true);
});
